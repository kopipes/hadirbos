...
import { LogOut, CheckCircle2, XCircle, RefreshCw, Clock, X } from 'lucide-react';
...
interface EarlyLeaveRequest {
  ...
  attendance: { id: string; date: string; checkIn?: string | null; checkOut?: string | null };
}
...
export default function EarlyLeaveApprovalPage() {
  const [requests, setRequests] = useState<EarlyLeaveRequest[]>([]);
  const [loading, setLoading] = useState(true);
  const [filterStatus, setFilterStatus] = useState('PENDING');
  const [reviewTarget, setReviewTarget] = useState<EarlyLeaveRequest | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/early-leave?${params}`);
  ...
  function openReview(req: EarlyLeaveRequest, action: 'APPROVED' | 'REJECTED') {
    setReviewTarget(req);
  ...
  async function handleReview() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/early-leave/${reviewTarget.id}`, {
        method: 'PATCH',
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(reviewAction === 'APPROVED' ? 'Izin pulang disetujui.' : 'Izin pulang ditolak.');
  ...
  return (
    <div className="space-y-5 max-w-4xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="page-title">Approval Izin Pulang Awal</h1>
      ...
      {loading ? (
        ...
        <div className="card text-center py-16 text-slate-400">
          ...
          <p className="font-medium">Tidak ada pengajuan izin pulang awal</p>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">
                      {req.user.nik} · {req.user.department || '-'}
                    </p>
                    ...
                    <div className="flex gap-4 mt-2 text-sm text-slate-600 flex-wrap">
                      ...
                      <span>Masuk: <strong>{req.attendance.checkIn ? formatTime(req.attendance.checkIn) : '-'}</strong></span>
                      {req.estimatedOut && (
                        <span>Est. Pulang: <strong>{req.estimatedOut}</strong></span>
                      )}
                    </div>

                    <p className="text-sm text-slate-700 mt-1.5 font-medium">Alasan: {req.reason}</p>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md animate-slide-up">
            <div className="p-6">
              ...
              <h2 className="text-lg font-bold text-slate-900 text-center">
                {reviewAction === 'APPROVED' ? 'Setujui Izin Pulang?' : 'Tolak Izin Pulang?'}
              </h2>
              ...
              <div className="mt-3 bg-gray-50 rounded-2xl p-4 space-y-1.5 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-slate-800">{reviewTarget.attendance.checkIn ? formatTime(reviewTarget.attendance.checkIn) : '-'}</span>
                </div>
                {reviewTarget.estimatedOut && (
                  <div className="flex justify-between">
                    <span className="text-slate-500">Est. Pulang</span>
                    <span className="font-semibold text-slate-800">{reviewTarget.estimatedOut}</span>
                  </div>
                )}
              ...
              {reviewAction === 'APPROVED' && (
                <div className="mt-3 flex items-start gap-2 p-3 bg-green-50 rounded-xl text-sm text-green-700">
                  ...
                  <p>Karyawan akan mendapat notifikasi dan dapat melakukan absen pulang sekarang.</p>
                </div>
              )}
